...
public class DormMain {
    public static void inputComebackTime(String roomNum, String name, Time comebackTime, boolean returnT) {
        try {
            ...
            String sql = "INSERT into comebacktime (roomNum, name, comebackTime, return) value(?, ?, ?, ?);";
    ...
    public static void ComebackTimeInfo() {
        try {
            ...
            while (rs.next()) {
                ...
                boolean returnT = rs.getBoolean("return");
                System.out.println(roomNum + "호 " + name + " " + comebackTime + " " + returnT);
            }
        } catch (ClassNotFoundException | SQLException e) {
            e.printStackTrace();
        }
